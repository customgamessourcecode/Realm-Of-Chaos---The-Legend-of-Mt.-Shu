custom_item_0405 = CustomItemSpellSystem:GetBaseClass()

local public = custom_item_0405
...
function public:OnCustomSpellStart(item)
	...
	ParticleManager:SetParticleControl(p, 2, Vector(0,255,127))
	target.__fire_particle = p
	target.__is_fire = true
	CustomNetTables:SetTableValue("Abilities", "custom_item_0405",  {can_not_cast=1})
	target:EmitSound("Item0182.Fire")
	SummonShuShanDiHen()
end

function public:CastFilterResultTarget( hTarget )
	if hTarget:GetUnitName() == "dihen_tower_05" then
		local t = CustomNetTables:GetTableValue("Abilities", "custom_item_0405")
