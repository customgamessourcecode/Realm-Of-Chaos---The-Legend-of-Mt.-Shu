custom_item_0402 = CustomItemSpellSystem:GetBaseClass()

local public = custom_item_0402
...
function public:OnCustomSpellStart(item)
	...
	ParticleManager:SetParticleControl(p, 2, Vector(0,191,255))
	target.__fire_particle = p
	target.__is_fire = true
	CustomNetTables:SetTableValue("Abilities", "custom_item_0402",  {can_not_cast=1})
	target:EmitSound("Item0182.Fire")
	SummonShuShanDiHen()
end

function public:CastFilterResultTarget( hTarget )
	if hTarget:GetUnitName() == "dihen_tower_02" then
		local t = CustomNetTables:GetTableValue("Abilities", "custom_item_0402")
